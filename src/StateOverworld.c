...
void START() {
    ...
    } else if (g_level_current >= 2 && g_level_current - 2 < NUM_PORTALS) {
        // Set position based on portal data, only if in bounds
        player_start_x = portal_positions[g_level_current-2].exit_tile_x;   
        player_start_y = portal_positions[g_level_current-2].exit_tile_y;
    } else {
        // Fallback/default position if out of bounds
        player_start_x = DEFAULT_OVERPLAYER_TILE_X;
        player_start_y = DEFAULT_OVERPLAYER_TILE_Y;
    }
        ...
        if (spr->type == SpriteOverPortal) {
            ...
            for (UINT8 j = 0; j < NUM_PORTALS; j++) {
                if (portal_tile_x == portal_positions[j].x && portal_tile_y == portal_positions[j].y) {
                    // Set the level field in the portal's custom data
                    //CUSTOM_DATA* data = (CUSTOM_DATA*)spr->custom_data;
                    //data->level = portal_positions[j].level;

                    // Enable/disable as before
